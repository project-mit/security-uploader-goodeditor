// 글꼴 배경색상 설정 함수
process.prototype.bgcolor = {
	version : '1.1',
	// 모듈 활성화 여부
	active : true,
	init : function(sTapRows)
	{
		var thisobj = e.$g('fontHiliteColorDiv_' + sTapRows);
		if (thisobj != null)
		{
	...
	action : function(obj, sTapRows)
	{
		...
		var _w = 170;
		var _h = 170;
		if (!e.$g('fontHiliteColorDiv_' + sTapRows)) {
			var H = 1;
			var S = 0;
			var V = 0;
			var drag = false;
			var colorDiv = e.$c('div');
				colorDiv.id = 'fontHiliteColorDiv_' + sTapRows;
				...
				colorDiv.innerHTML =	'<div id="fontColor_Area_' + sTapRows + '" style="margin:5px 0 0 5px; float:left; position:relative; width:' + _w + 'px; height:' + _h + 'px; background:#f00 url(' + obj.config.img_dir + '/nColorPlug.png) no-repeat;">' +
										...
										'	<input type="button" value="입력" style="float:left; width:31px; font-size:11px; font-family:돋움; border:1px solid #777; padding:2px 2px 0 2px;" onclick="buttonEvent.setHiliteColor(e.$g(\'fontColor_' + sTapRows + '_text\').value); goodEditor.editor.toolsObjectsInit();" />' +
										'</div>';
				...
				e.event(e.$g('fontColor_Box_' + sTapRows), 'mouseup', function(event){
					drag = false;
				...
				for(var i = 360; i >= 0; i-=4.2) {
					innStr += '<div style="background:#' + hsv2rgb(i, 100, 100) + '; height:2px; font-size:1px; margin:0 0;"></div>';
				}
				o.innerHTML = innStr;
		} else {
			if (e.$g('fontHiliteColorDiv_' + sTapRows).style.display == 'block') {
				e.$g('fontHiliteColorDiv_' + sTapRows).style.display = 'none';
			} else {
				e.$g('fontHiliteColorDiv_' + sTapRows).style.display = 'block';
			}
		}
	}
};
...
// 해당 기능을 담당하는 액션 함수
button.prototype.setHiliteColor = function(hiliteColor)
{
	...
	if(e.IE == 1){
		...
		rang.execCommand('backcolor', false, hiliteColor);
		obj.contentWindow.focus();
	}else{
		obj.contentWindow.document.execCommand('hilitecolor', false, hiliteColor);
	}
}
